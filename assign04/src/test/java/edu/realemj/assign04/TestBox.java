...
public class TestBox {
    ...
    public void test_draw_one(Box b, Integer index) {
        // Create map
        ScreenBuffer singleMap = new ScreenBuffer(5, 10, '.');
        // Get desired output for a SINGLE draw
        String desiredSingle = allSingleMapOutputs[index];
        // Draw on map
        b.draw(singleMap);
        // Compare results
        Assert.assertEquals(singleMap.getDisplayString(), desiredSingle);
    }

    @Test()
    public void test_draw_combo() {
        // Create map
        ScreenBuffer comboMap = new ScreenBuffer(5, 10, '.');
        ...
        for(int i = 0; i < allBoxes.length; i++) {
            ...
            Assert.assertEquals(comboMap.getDisplayString(), desiredOutput, "Up to Box at index " + i + ":");
        }
    }
}
