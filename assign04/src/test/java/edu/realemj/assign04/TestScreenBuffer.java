...
public class TestScreenBuffer {
    ...
    public Object[][] mapGen() {
        ...
        for(int rows = 1; rows <= numRows; rows++) {
            for(int cols = 1; cols <= numCols; cols++) {
                maps[index][0] = new ScreenBuffer(rows, cols, defaultChar);
                maps[index][1] = rows;
                maps[index][2] = cols;
    ...
    public void test_getRowCnt(ScreenBuffer myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
        Assert.assertEquals(myMap.getRowCnt(), rowCnt.intValue());        
    }

    @Test(dataProvider = "mapGen")
    public void test_getColCnt(ScreenBuffer myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
        Assert.assertEquals(myMap.getColCnt(), colCnt.intValue());        
    }

    @Test(dataProvider = "mapGen")
    public void test_isValidPosition(ScreenBuffer myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
        // Check negative
    ...
    @Test(dataProvider = "mapGen")
    public void test_GetAndSetPos(ScreenBuffer myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
    ...
    public void test_toString() {
        ScreenBuffer m = new ScreenBuffer(5, 10, 'a');
        Assert.assertEquals(m.toString(), "5 x 10 ScreenBuffer (default: a)");

        m = new ScreenBuffer(7, 8, '@');
        Assert.assertEquals(m.toString(), "7 x 8 ScreenBuffer (default: @)");
    }
